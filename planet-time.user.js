...
function icon_tag (icon_key, ttip) {
  var ttip_html = ' title="'+(ttip||'');
  return '<img src="data:image/png;base64,'+icons[icon_key]+'"'+(ttip ? ttip_html : '')+'">';
}//icon_tag

function setup_cartellino () {
  var t = $('#gvCartellino_DXMainTable');
  
  // formatta una sola volta
  if (t.length > 0 && t.hasClass('tpunipd-formatted'))
    return;
  t.addClass('tpunipd-formatted');
  ...
  var moi_tot = 0;
  $("#lblTotalizzatori tr td:first-child:contains('MO0')").parent().children(':last-child').each(function () {
    var s = $(this).text().trim().replace(' ', '').split(':');
    moi_tot += parseInt(s[0])*60 + (s[0][0] == '-' ? -parseInt(s[1]) : parseInt(s[1]));
  });
  var moi = (Math.sign(moi_tot) < 0 ? '-' : '') + Math.trunc(Math.abs(moi_tot) / 60) + ':' + ('00'+(Math.abs(moi_tot) % 60)).slice(-2);
  var pasti = $("#lblTotali td:contains('Buoni pasto maturati')").parent().children().last().prev().text();
  var ferie = [], tot_ferie = 0;
  $.each(['FE03', 'FE02', 'FE01'], function () {
    var n = $("#lblTotalizzatori tr td:first-child:contains('"+this+"')").parent().children().last().text().replace(',00', '');
    ferie.push(n);
    tot_ferie += parseInt(n);
  });
  $('#tb > ul .dxm-alignSpacing').
    css('vertical-align', 'middle').
    css('font-family', 'monospace').
    html(
      '<div style="background-color: #ddd; display: inline-block; border: 1px inset black; padding: 0.1rem; margin-left: 4rem; border-radius: 0.5rem">'+
      '<span title="MOI anno corrente">'+icon_tag('tb_moi')+' '+moi+'</span>'+
      '<span title="FERIE tot (corrente + precedente + scad.30-06)">'+icon_tag('tb_ferie')+' '+tot_ferie+' ('+ferie.join('+')+')</span>'+
      '<span title="Buoni pasto maturati">'+icon_tag('tb_pasto')+' '+pasti+'</span>'+
      '</div>'
    ).find('img').css('vertical-align', 'text-bottom').end().
      find('span').css('padding', '0 1rem');
  
  // imposta un'altezza fissa al cartellino centrale
  $('div.dxgvCSD').css('overflow', 'auto').css('height', '50vh');

  // mostra icona del buono pasto
  t.find('tbody tr.cart_cell div.pasto').
    replaceWith(icon_tag('pasto', 'buono pasto maturato'));

  // evidenzia riposi
  t.find('tbody tr.cart_cell td:nth-child(2)[style*="rgb(0, 0, 255)"],'+
         'tbody tr.cart_cell td:nth-child(2)[style*="#0000ff"]').
    parent().css('background-color', 'lightblue');
  t.find('tbody tr.cart_cell td:nth-child(2)[style*="rgb(255, 0, 0)"],'+
         'tbody tr.cart_cell td:nth-child(2)[style*="#ff0000"]').
    parent().css('background-color', 'lightpink');
  
  // evidenzia richieste effettuate
  t.find('tbody tr.cart_cell td:nth-child(2)').each(function () {
    if ($(this).find('sup').length > 0)
      $(this).css('background-color', 'orange').attr('title', $(this).find('sup').text().trim());
  });
  
  // compatta giorno + monospace
  t.find('tbody tr.cart_cell td:nth-child(2)').
    css('font-family', 'monospace').
    each(function (i) { $(this).text( $(this).text().trim().replace(' ', '').substr(0,3) ); });
  
  // mostra icone per l'assenza (ferie/recupero/TL/agile)
  t.find('tbody tr.cart_cell td.dxgvPHEC').each(function (i) {
    var el = $(this),
        giustificativi = [],
        gg = el.parent().find('td:nth-child(2)');
    
    var g = el.prev().text().trim();
    if (g.match(/^C - /))
      giustificativi.push(g);
    ...
    if (giustificativi.length > 1)
      // mostra errore se c'é piú di un giustificativo
      gg.append(' '+icon_tag('warn', giustificativi.join(', ')));
    else if (giustificativi.length == 1) {
  ...
  $.each(['ECCE', 'PPAUM', 'ORDI', 'DEB'], function (i, key) {
    t.find('tbody tr.cart_cell td:contains('+key+')').each(function () {
      $(this).
...
(function ($) { $(function () {
  setInterval(setup_cartellino, 1000);
