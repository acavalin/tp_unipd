// ==UserScript==
// @name        RedMine SCM auto login (unipd)
// @namespace   camelsoft
// @description Inserisce le credenziali automaticamente
// @include     https://scm.csia.unipd.it/redmine/login*
// @include     https://scm.csia.unipd.it/redmine/
// @include     https://scm.csia.unipd.it/redmine/my/page
// @require     https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js
// @icon        data:image/png;base64,
// @version     1.0.4
// @grant       none
// ==/UserScript==

jQuery.noConflict();

(function ($) { $(function () {
// -----------------------------------------------------------------------------
if (window.location.pathname == '/redmine/' ||
    window.location.pathname == '/redmine/my/page')
  window.location = '/redmine/projects/XXX/repository';

if (window.location.pathname == '/redmine/login') {
  $('#username').val('username');
  $('#password').val('password');
  //$('#autologin').prop('checked', true); // brokes login process
  $('#password').parents('form:first').submit();
}//if
// -----------------------------------------------------------------------------
});})(jQuery);
