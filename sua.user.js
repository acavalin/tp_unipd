...
if (window.location.toString().match(/https:\/\/off270.mur.gov.it\/off270\/sua\d\d\/sua20\d\d.php\?parte=20&.+/)) {
  ...
  $('#COMUNICAZIONE, #DMACCREDITAMENTO').
    css('position', 'absolute').css('right', '1rem').css('top', '0').
    css('width', '48%').
    css('z-index', '1000').
    css('border', '2px inset black').
    css('height', '40vh').
    css('overflow', 'scroll').
    prepend('<div class="mytitle" style="font-size: 1.75rem; text-align: center; background-color: yellow; position: sticky; top: 0; width: 100%;">COMUNICAZIONI</div>').
    prev().hide();
  $('#DMACCREDITAMENTO').
    css('top', 'calc(40vh + 1rem)').
    find('.mytitle').text('ACCREDITAMENTO');
  $('#DMACCREDITAMENTO').nextAll('[id]').css('width', '30%');
...
if (window.location.toString().match(/https:\/\/off270.mur.gov.it\/off270\/sua\d\d\/docenti_di_riferimento.php\?.+/)) {
  ...
  $('<button type="button" id="bt_scrape">⛏️ Compila TSV</button>').appendTo('#tp_bts').click((ev) => {
    ...
    let csv = [
      'ID', 'Classe', 'Corso', 'N.Sedi', 'N.Studenti', 'N.Docenza',
