...
if (window.location.toString().match(/https:\/\/off270.mur.gov.it\/off270\/sua\d\d\/elenco_classi.php\?parte=2&.+/)) {
  ...
  $('<button type="button" id="bt_scrape_vepd">⛏️ Recupera dati Verifica ex-post docenza</button> ').appendTo('#tp_bts').click((ev) => {
    ev.preventDefault();
    let bt = $(ev.target).attr('disabled', true);
    bt.text( bt.text() + ' [...]');
    ...
    $.ajax({
      ...
      success: (data, status, xhr) => {
        ...
        let headers =  $.makeArray( t.find('thead:first th:gt(0)').map((i, el) => { return el.innerText; }) );
        headers.splice( 2, 0, 'Ciclo concluso', 'Primo Anno');
        headers.splice(10, 2, 'Docenti necessari ExP', 'Professori necessari ExP',
        ...
        bt.data('headers', headers);
        bt.data('dati'   , dati);


        bt.text( bt.text().replace(/ \[.+\]/, ''));

        alert('Dati recuperati, ora premi l\'altro pulsante');
      },//success
      error: (xhr, status, err) => {
  ...
  $('<button type="button" id="bt_scrape">⛏️ Recupera dati CdS</button>').appendTo('#tp_bts').click((ev) => {
    ...
    let vepd = { headers: $('#bt_scrape_vepd').data('headers'),
                 dati:    $('#bt_scrape_vepd').data('dati') };

    let csv = [
      'ANNO', 'RAD', 'SUA', 'CODICIONE', 'CODICE_INTERNO', 'CODICE', 'DESCR',
