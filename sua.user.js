// ==UserScript==
// @name        SUA enhancements (unipd)
// @namespace   camelsoft
// @description Miglioramenti alla scheda SUA del MIUR
// @include     https://ava.mur.gov.it/
// @include     https://off270.mur.gov.it/*
// @icon        data:image/x-icon;base64,
// @downloadURL https://github.com/acavalin/tp_unipd/raw/main/sua.user.js
// @require     https://ajax.googleapis.com/ajax/libs/jquery/3.6.1/jquery.min.js
// @version     2.7.0
// @grant       GM_xmlhttpRequest
// @connect     self
// @license     GPLv3
// ==/UserScript==

// https://www.tampermonkey.net/documentation.php?locale=en#api:GM_xmlhttpRequest
// https://www.tampermonkey.net/documentation.php?locale=en#meta:connect

(function ($) { $(function () {
// -----------------------------------------------------------------------------
const tp = {
  ajax: opts => {
    opts.beforeSend?.();

    let method = opts.method || 'GET';
    let params = new URLSearchParams(opts.data || {});
    GM_xmlhttpRequest({
      method:       method,
      responseType: opts.dataType,
      url:          method == 'GET' ? `${opts.url}?${params.toString()}` : opts.url,
      data:         method == 'GET' ? null : new URLSearchParams(opts.data),
      user:         opts.user,
      password:     opts.password,
      nocache:      opts.cache == null ? null : !opts.cache,
      timeout:      opts.timeout,
      onload:    function(resp){ opts.success?.(resp.response||resp.responseText, resp, opts.error); opts.complete?.(resp); },
      onerror:   function(resp){ opts.error?.('error'  , resp); opts.complete?.(resp); },
      ontimeout: function(resp){ opts.error?.('timeout', resp); opts.complete?.(resp); },
      onabort:   function(resp){ opts.error?.('abort'  , resp); opts.complete?.(resp); }
    });
  },//ajax

  dl_link: (label, fname, content) => {
    var el = document.createElement('a');
    el.setAttribute('href', 'data:application/octet-stream;charset=UTF-8,' + encodeURIComponent(content));
    return $(el).attr('download', fname).text(label);
  },//dl_link

  // https://stackoverflow.com/questions/17415579/how-to-iso-8601-format-a-date-with-timezone-offset-in-javascript/17415677#17415677
  ts: (date) => {
    date ||= new Date();
    return  '@' + [
      date.getFullYear() - 2000,
      `0${date.getMonth() + 1}`.slice(-2),
      `0${date.getDate()     }`.slice(-2),
    ].join('') + 'T' + [
      `0${date.getHours()    }`.slice(-2),
      `0${date.getMinutes()  }`.slice(-2),
      //`0${date.getSeconds()  }`.slice(-2),
    ].join('');
  },//ts

  text_nodes: (node, content=false) => {
    return $(node).contents().
      filter((i, el) => { return el.nodeType == 3 && el.textContent?.trim()?.length > 0; }).
      map((i, el) => { return content ? el.textContent.trim() : el; });
  },//text_nodes

  all_texts: (node) => {
    return $(node).contents().map((i, el) => { return el.textContent.trim(); });
  },//all_text

  rem2px: (rem) => {
    return parseInt(parseFloat(rem) * parseFloat(getComputedStyle(document.documentElement).fontSize));
  },//rem2px
};

// autofill login form in homepage
if (window.location == 'https://ava.mur.gov.it/') {
  $('select[name="ateneo"]').val(19); // Univ. di PADOVA
  $('input[name="username"]').val('lauree_19');
  $('input[name="password"]').focus().get(0).select();
  //$('input[name="password"]').val('xxx').get(0).form.submit();
}//if

// SUA-CdS 20XX
// https://off270.mur.gov.it/off270/sua25/sua2025.php?parte=20&vis_pdf=&user=ATElauree_19
if (window.location.toString().match(/https:\/\/off270.mur.gov.it\/off270\/sua\d\d\/sua20\d\d.php\?parte=20&.+/)) {
  // manda testo lungo accapo
  $('#COMUNICAZIONE').nextAll('[id]').css('width', '30%');

  // metti a destra in un box scrollabile le sezioni Comunicazioni e Accreditamento
  let top_offset = $('#content').offset().top + tp.rem2px(1);
  $('#COMUNICAZIONE, #DMACCREDITAMENTO').
    css('position', 'fixed').css('right', '1rem').
    css('top', top_offset).
    css('width', '48%').
    css('z-index', '1000').
    css('border', '2px inset black').
    css('height', '40vh').
    css('overflow', 'scroll').
    prepend('<div class="mytitle" style="font-size: 1.75rem !important; text-align: center; background-color: yellow; padding: 0.5rem; position: sticky; top: 0; width: 100%;">COMUNICAZIONI</div>').
    prev().hide();
  $('#DMACCREDITAMENTO').
    css('top', 'calc(40vh + '+(top_offset + tp.rem2px(0.5))+'px)').
    find('.mytitle').text('ACCREDITAMENTO');

  let anno = parseInt(window.location.toString().replace(/.+sua(\d\d)\/.+/, '20$1'));
  if (anno >= 2026) {
    $('#inizio > .anchor-badges:first').hide();
  } else {
    // allinea a sx il box con tutti i links
    $('#inizio > .table-tab').css('position', 'absolute').css('left', '0').css('top', '0');
  }//if-else

  // sposta testo dentro ai link con l'iconcina
  $('#inizio .table-tab h3 a[href*=".php"] > img').map((i, el) => {
    let tn = tp.text_nodes( $(el).parent().parent() )[0];
    $(tn).insertAfter(el);
  });
}// if SUA-CdS 20XX

// SUA-CdS 20XX > Docenti di riferimento e Figure Specialistiche
// es: https://off270.mur.gov.it/off270/sua25/docenti_di_riferimento.php?user=ATElauree_19
if (window.location.toString().match(/https:\/\/off270.mur.gov.it\/off270\/sua\d\d\/docenti_di_riferimento.php\?.+/)) {
  // pulsantiera
  $('<div id="tp_bts" style="text-align: right"></div>').prependTo('#scelta');

  $('<button type="button" id="bt_scrape">⛏️ Compila TSV</button>').appendTo('#tp_bts').click((ev) => {
    ev.preventDefault();
    $(ev.target).attr('disabled', true);

    let csv = [
      'ID', 'Classe', 'Corso', 'Numero Sedi', 'Studenti ai fini del calcolo', 'Docenza necessario',
      'CF', 'Nome', 'Ruolo', 'SSD', 'Peso',
      'Anomalie', 'Fig.Spec.nec', 'Fig.Spec.ins', 'Tutor'
    ]
    csv = csv.join("\t");

    let sep = $('<i>&nbsp;&nbsp;</i>').get(0).textContent;
    $('#scelta table tr:gt(0)').each(function (i, tr) {
      tr = $(tr);
      let columns = [];
      tr.find('td').each((i, el) => { columns.push(el.textContent.trim()); });
      let row1 = columns.slice(1,7); // 1~7
      let row3 = columns.slice(8);   // >8

      tr.find('td:eq(7) li > div').each(function (i, div) {
        // es. { 0: "", 1: "CNDLNEXXXXXXXXXX", 2: "CANADELLI Elena", 3: "PA  M-STO/05  1", 4: "" }
        let docente = tp.all_texts(div);
        let docente_dett = docente[3].split(sep);
        let row2 = [
          docente[1]     ?.toString() || '--', // CF
          docente[2]     ?.toString() || '--', // Nome
          docente_dett[0]?.toString() || '--', // Ruolo
          docente_dett[1]?.toString() || '--', // SSD
          docente_dett[2]?.toString() || '--', // Peso
        ];

        let row = row1.concat(row2).concat(row3);
        $(div).attr('data-info', JSON.stringify(row));
        //console.debug(row);

        csv += "\n" + row.join("\t")
      });
    });

    // abbiamo terminato lo scrape di tutti i cds
    let anno = 2000 + parseInt(window.location.pathname.replace(/.+\/sua(\d{2})\/.+/, '$1'));
    tp.dl_link('💾 Scarica TSV', `${anno}_SUA_doc_rif__${tp.ts()}.csv`, csv).
      appendTo( $('#tp_bts').append('&nbsp;') ).
      get(0).click();
  });
}// if SUA-CdS 20XX > Docenti di riferimento e Figure Specialistiche

// SUA-CdS 20XX > Visualizza Schede
// es: https://off270.mur.gov.it/off270/sua25/elenco_classi.php?parte=2&anno=2025&vis_pdf=&user=ATElauree_19
if (window.location.toString().match(/https:\/\/off270.mur.gov.it\/off270\/sua\d\d\/elenco_classi.php\?parte=2&.+/)) {
  class SuaCdsScraper {
    constructor (cds_node, callback) {
      this.ajax_timeout = 20000;
      this.call_sequence = ['scrape_cod_interno_e_classi', 'scrape_pds', 'scrape_sedi', 'scrape_finished'];
      this.on_complete = callback || function (data) { return null; };

      this.data = {
        anno: 2000 + parseInt(window.location.pathname.replace(/.+\/sua(\d{2})\/.+/, '$1')),
        pds: [], classi: [], sedi: [],
      };

      this.data.descr = tp.text_nodes(cds_node.find('div.titolo'), {content: true})[0];

      console.info(`SCRAPING: ${this.data.descr}`);

      // estrai i 3 ID, es: "(ID SUA=1610825 - ID RAD=1388185 - CODICIONE=0280107301800003)"
      cds_node.find('div.titolo p:eq(0)').text().trim().replace(/[()]/g, '').split(' - ').forEach(pair => {
        let [k, v] = pair.toLowerCase().replaceAll(' ', '_').split('=');
        this.data[k.trim()] = v.trim();
      });
      // estrai nota sulla replica
      this.data.replica = cds_node.find('div.titolo p:eq(1)').text().trim();
      console.debug(`  - ${this.data.id_sua} | ${this.data.id_rad} | ${this.data.codicione}`);

      this[ this.call_sequence.shift() ]?.();
    }//constructor

    // Compila scheda > Amministrazione > Altre Informazioni > Altre Informazioni - Codice interno
    // es. https://off270.mur.gov.it/off270/sua25/riepilogo.php?ID_RAD=1610802&sezione_aq=A&vis_quadro=I&user=ATElauree_19
    scrape_cod_interno_e_classi () {
      const self = this;
      $.ajax({
        url: `https://off270.mur.gov.it/off270/sua${self.data.anno - 2000}/riepilogo.php?ID_RAD=${self.data.id_sua}&sezione_aq=A&vis_quadro=I&user=ATElauree_19`,
        dataType: 'html', cache: false, timeout: self.ajax_timeout,
        success: (data, status, xhr) => {
          if (data.match(/ACCESSO NEGATO/i)) {
            console.log(`ERR ID_SUA=${self.data.id_sua} scrape_cod_interno - Accesso negato`);
            return;
          }//if

          let doc = $(data);

          // estrai le classi ministeriali, es "LM-17 R - Fisica & LM-58 R - Scienze dell'universo"
          doc.find('#descorso-int div.titolo-corso').text().trim().split('&').map((s) => {
            let [sigla, descr] = s.trim().split(/ - +(.+)/);
            this.data.classi.push({ sigla: sigla?.trim(), descr: descr?.trim() });
            console.debug(`  - classe: ${sigla} | ${descr}`);
          });

          if (!data.match(/Codice interno all'ateneo del corso/i))  {
            console.log(`ERR ID_SUA=${self.data.id_sua} scrape_cod_interno - Codice interno non trovato`);
            return;
          }//if

          self.data.codice_interno = $(data).find('div#TI1T strong:contains("Codice interno")').parent().next().text().trim();
          console.debug(`  - cod int: ${self.data.codice_interno}`);
        },//success
        error: (xhr, status, err) => {
          console.log([`ERR ID_SUA=${self.data.id_sua} scrape_cod_interno (${status} | ${err})`, xhr]);
        },//error
        complete: () => { self[ self.call_sequence.shift() ]?.(); }
      });
    }//scrape_cod_interno_e_classi

    // Compila scheda > Amministrazione > Cruscotto controlli - Eventuali curriculum
    // es. https://off270.mur.gov.it/off270/sua25/riepilogo.php?ID_RAD=1610828&sezione_aq=A&vis_quadro=SC&user=ATElauree_19
    scrape_pds () {
      const self = this;
      $.ajax({
        url: `https://off270.mur.gov.it/off270/sua${self.data.anno - 2000}/riepilogo.php?ID_RAD=${self.data.id_sua}&vis_quadro=&sezione_aq=A&user=ATElauree_19`,
        dataType: 'html', cache: false, timeout: self.ajax_timeout,
        success: (data, status, xhr) => {
          if (data.match(/ACCESSO NEGATO/i)) {
            console.log(`ERR ID_SUA=${self.data.id_sua} scrape_pds - Accesso negato`);
            return;
          }//if

          if (data.match(/Non sono previsti curricula/i)) return;

          let doc = $(data).find('div#TSC10T table');
          doc.find('tr').map((i, tr) => {
            let info = $(tr).find('td').map((j, td) => { return $(td).text(); });
            let sigla = info[1].toString().match(/(.+)\^(.+)\^(.+)\^(.+)/)?.[3] || '--';
            self.data.pds.push({ descr: info[0]?.trim(), cod: info[1]?.trim(), sigla: sigla?.trim() });
            console.debug(`  - pds: ${sigla} | ${info[1]} | ${info[0]}`);
          });
        },//success
        error: (xhr, status, err) => {
          console.log([`ERR ID_SUA=${self.data.id_sua} scrape_pds (${status} | ${err})`, xhr]);
        },//error
        complete: () => { self[ self.call_sequence.shift() ]?.(); }
      });
    }//scrape_pds

    // Compila scheda > Amministrazione > Cruscotto controlli - Sede
    // https://off270.mur.gov.it/off270/sua25/cruscotto_controlli.php?ID_RAD=1610802&parte=1&vis_pdf=&user=ATElauree_19
    scrape_sedi () {
      const self = this;
      $.ajax({
        url: `https://off270.mur.gov.it/off270/sua${self.data.anno - 2000}/cruscotto_controlli.php?ID_RAD=${self.data.id_sua}&parte=1&vis_pdf=&user=ATElauree_19`,
        dataType: 'html', cache: false, timeout: self.ajax_timeout,
        success: (data, status, xhr) => {
          if (data.match(/ACCESSO NEGATO/i)) {
            console.log(`ERR ID_SUA=${self.data.id_sua} scrape_sede - Accesso negato`);
            return;
          }//if

          if (!data.match(/Controlli su Sedi - Repliche/i))  {
            console.log(`ERR ID_SUA=${self.data.id_sua} scrape_sede - Sedi non trovate`);
            return;
          }//if

          // Il Corso ha sede nel seguente comune: 028060 - PADOVA (PD)
          // Il Corso ha sedi nei seguenti 8 comuni: 025021 - FELTRE (BL), 028055 - MONSELICE (PD), ...
          let testo_sedi = $(data).
            find('div#content-tabs-blu td:contains("Controlli su Sedi")+td li').
            filter((i, el) => { return $(el).text().trim().match(/il corso ha sed[ei] ne[li] seguent[ei].+comun[ei]/i); }).
            text().trim().split(': ')?.[1] || '';
          testo_sedi.split(', ').map((el, i) => {
            let [cod, descr] = el.split(' - ') || ['--', ''];
            self.data.sedi.push({ cod: cod?.trim(), descr: descr?.trim() });
            console.debug(`  - sede: ${cod} | ${descr}`);
          });
        },//success
        error: (xhr, status, err) => {
          console.log([`ERR ID_SUA=${self.data.id_sua} scrape_sede (${status} | ${err})`, xhr]);
        },//error
        complete: () => { self[ self.call_sequence.shift() ]?.(); }
      });
    }//scrape_sedi

    scrape_finished () {
      this.on_complete(this.data);
      this[ this.call_sequence.shift() ]?.();
    } // scrape_finished
  }//class SuaCdsScraper

  // pulsantiera
  $('<div id="tp_bts" style="text-align: right"></div>').prependTo('#scelta');

  $('<button type="button" id="bt_scrape">⛏️ Recupera dati CdS</button>').appendTo('#tp_bts').click((ev) => {
    ev.preventDefault();
    $(ev.target).attr('disabled', true);

    let anno = 2000 + parseInt(window.location.pathname.replace(/.+\/sua(\d{2})\/.+/, '$1'));
    console.info(`SCRAPING: verifica ex-post docenza ${anno}`);

    let vepd = { headers: [], dati: {} /* { id_sua: [ ... ], ... } */ };

    // SUA-CdS 20XX > Corsi (20XX-1)/XX - verifica ex-post docenza
    // https://off270.mur.gov.it/off270/sua25/expost.php?parte=1000&vis_pdf=&user=ATElauree_19
    $.ajax({
      url: `https://off270.mur.gov.it/off270/sua${anno + 1 - 2000}/expost.php?parte=1000&vis_pdf=&user=ATElauree_19`,
      dataType: 'html', cache: false, timeout: 10000,
      async: false,
      success: (data, status, xhr) => {
        if (!data.match(/table-expost/i))  {
          console.log(`ERR ExP-DOC - tabella non trovata`);
          return;
        }//if

        let t = $(data).find('table.table-expost');

        vepd.headers = $.makeArray( t.find('thead:first th:gt(0)').map((i, el) => { return el.innerText; }) );
        vepd.headers.splice( 2, 0, 'Ciclo concluso', 'Primo Anno');
        vepd.headers.splice(10, 2, 'Docenti necessari ExP', 'Professori necessari ExP',
                              'Docenti inseriti ExP',  'Professori inseriti ExP');

        let tot = t.find('tbody tr').length;

        t.find('tbody tr').each((i, tr) => {
          let campi = $.makeArray( $(tr).find('td:gt(0)').map((i, el) => {
            if (i == 1) {
              let result = el.innerText.split("Ciclo concluso: ");
              return result.concat( result.pop().split(" - Primo Anno: ") );
            } else
              return el.innerText.split(' / ');
          }) ).map((el, i) => { return el.trim(); });

          vepd.dati[ campi[0] ] = campi;
        });
      },//success
      error: (xhr, status, err) => {
        console.log([`ERR ExP-DOC - GET (${status} | ${err})`, xhr]);
      }//error
    });

    let csv = [
      'ANNO', 'RAD', 'SUA', 'CODICIONE', 'CODICE_INTERNO', 'CODICE', 'REPLICA', 'DESCR',
      'SEDI_CODICE', 'SEDI_DESCR',
      'PDS_SIGLA', 'PDS_CODICE', 'PDS_DESCR',
      'CLASSI_SIGLA', 'CLASSI_DESCR'
    ]
    if (vepd.headers) csv = csv.concat(vepd.headers.slice(2));
    csv = csv.join("\t");

    (function scrape_next_node () {
      let node = $('div.corso:not(.scraped):first').addClass('scraped');

      // abbiamo terminato lo scrape di tutti i cds
      if (node.length == 0) {
        let anno = 2000 + parseInt(window.location.pathname.replace(/.+\/sua(\d{2})\/.+/, '$1'));
        tp.dl_link('💾 Scarica TSV', `${anno}_SUA_cds__${tp.ts()}.csv`, csv).
          appendTo( $('#tp_bts').append('&nbsp;') ).
          get(0).click();
        return;
      }//if

      // update counter
      let [n, tot] = [$('div.corso.scraped').length, $('div.corso').length];
      $('#bt_scrape').text( $('#bt_scrape').text().replace(/\[.+\]/, '') + ` [${n}/${tot}]`);

      // mark cds as WIP
      node.find('#descorso-int').css('border-left', '1rem solid coral');
      node.find('.titolo').prepend('<span class="scrape-icon">⏳</span>');

      new SuaCdsScraper(node, (data) => {
        node.attr('data-info', JSON.stringify(data));

        // mark cds as DONE
        node.find('#descorso-int').css('border-left', '1rem solid lightgreen');
        node.find('.scrape-icon').text('✅');

        console.debug(data);

        row = [
          data.anno          ?.toString() || '--',
          data.id_rad        ?.toString() || '--',
          data.id_sua        ?.toString() || '--',
          data.codicione     ?.toString() || '--',
          data.codice_interno?.toString() || '--',
          data.codice_interno?.toString()?.slice(0,6) || '--',
          data.replica       ?.toString() || '--',
          data.descr         ?.toString() || '--',
          data.sedi  .map((el, i) => { return el.cod;   })?.join('; ') || '--',
          data.sedi  .map((el, i) => { return el.descr; })?.join('; ') || '--',
          data.pds   .map((el, i) => { return el.sigla; })?.join('; ') || '--',
          data.pds   .map((el, i) => { return el.cod;   })?.join('; ') || '--',
          data.pds   .map((el, i) => { return el.descr; })?.join('; ') || '--',
          data.classi.map((el, i) => { return el.sigla; })?.join('; ') || '--',
          data.classi.map((el, i) => { return el.descr; })?.join('; ') || '--',
        ]
        if (vepd.dati) row = row.concat(vepd.dati[data.id_sua]?.slice(2));
        csv += "\n" + row.join("\t")

        scrape_next_node();
      });
    })();
  });

  $('#tp_bts button').css('margin-left', '0.2rem');
}//if - SUA-CdS 20XX > Visualizza Schede
// -----------------------------------------------------------------------------
});})(jQuery.noConflict());
